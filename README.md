...
> **현재 상태:** 예약 등록 · 설정 조회·변경 · 지연·실패 주입까지 구현됐다. 조회 · 취소 · 결함 주입 ·
> 초기화는 아래 계약대로 구현할 예정이다.
...
**Docker 가 켜져 있으면 MySQL 시험(Testcontainers)도 같이 돈다.** 없으면 그 시험만 건너뛰므로,
동시성을 건드렸으면 Docker 를 켜고 돌린다.
